...
export default function PlaceDetailSheet({
  ...
  // [예측 프리패칭과 연동] MapCanvas의 onPrefetchPlace가 마커 탭 시점에 이미 queryClient.prefetchQuery를
  // 호출해두므로, 시트가 열리는 즉시 이 useQuery는 대부분 캐시 히트로 즉시 렌더링된다.
  const tourInfo = useQuery({
    queryKey: ["tourIntro", place?.placeId],
    queryFn: () => fetchTourIntro(place!.placeId.replace("tour-", "")),
    enabled: Boolean(place && place.source === "tourapi"),
    staleTime: 5 * 60_000,
  });
  ...
  const naverHomepage = useQuery({
    ...
    enabled: Boolean(place && place.source === "kakao" && !place.homepageDirect),
    staleTime: 5 * 60_000,
  });

  const subDef = place ? SUB_DEFS[place.categoryDepth1]?.find((s) => s.id === place.categoryDepth2) : null;
    ...
    <AnimatePresence>
      {place && (
        ...
        >
          ...
          {/* [Sticky CTA] 스크롤과 무관하게 시트 최하단 고정, 네온 옐로우 배경 + 딥 블루 텍스트, 탭 시 scale 0.95 햅틱 애니메이션 */}
          <motion.button
            whileTap={{ scale: 0.95 }}
