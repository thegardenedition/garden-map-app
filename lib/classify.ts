...
const EXCLUDE_KEYWORDS = [
  ...
  "화환", "절화",
  // [신규 — 2026-09-16, 실사용자 신고로 확인] "한국소방ENG 소방스프링클러..."가 "자재/잔디/
  // 석재"로 노출된다는 신고를 받고 D1을 직접 조회해 세 가지 새는 구멍을 확인했다.
  // (1) 소방설비 업체 16건(전부 소방설비/스프링클러/화재감지 시공사, 조경과 무관).
  // (2) 공원·수목원·인조잔디구장·화훼공판장 등 "장소"의 부속 시설(주차장·화장실·매점)이
  //     그 장소 이름을 그대로 물려받으면서 "잔디"/"정원"/"화훼" 같은 긍정 키워드에 걸려
  //     업체처럼 노출됨 — 예: "화성비봉인조잔디구장 주차장", "허브나라농원 정문주차장"
  //     (주차장만 9건), "허브나라 농원 화장실", "자갈밭매점". 실제로는 그 장소의 편의시설일
  //     뿐 조경 업체가 아니다.
  // (3) 같은 이유로 전기차 충전소 13건도 노출(예: "제2월곡인조잔디구장 전기차충전소").
  // 셋 다 D1의 grp/sub가 과거 대량 수집 때 이미 잘못 저장돼 있어 classifyBiz(실시간 분류)를
  // 안 거치므로, 이름 기반 최종 안전망(isExcludedName)에서만 잡을 수 있다.
  "소방", "충전소", "충전기", "주차장", "화장실", "매점",
];
